...
public:
    CPUAndScheduler(std::shared_ptr<ConfigValues> cnfg,
        ...
        std::shared_ptr<BackingStoreEmulator> bs,
        std::shared_ptr<PagingMemoryAllocator> malloc);
    void CPUScheduler();
    void batchProcessor();
    ...
    std::uint32_t getAmountOfActiveCPUs();
    std::uint32_t getAmountOfTotalCPUs();
    std::vector<std::uint32_t> getCPUCycleStats() const;
...
private:
    ...
    std::shared_ptr<BackingStoreEmulator> backingStore;
    std::shared_ptr<PagingMemoryAllocator> memoryAllocator;
